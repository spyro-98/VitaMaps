...
constexpr const char *kItalian[] = {
    ...
    "Comportamento HUD", "Scala della mappa", "Mirino centrale", "Unità di misura",
    "Animazioni interfaccia", "Log persistente su memoria Vita",
    "Auto: 2,5 s", "Sempre visibile", "Con l'HUD", "Visibile", "Nascosto", "Imperiale",
    ...
    "ruota", "Modalità escursione", "Importa/esporta GPX", "In arrivo",
    "Cronologia", "importa", "esporta", "aggiorna", "Importati %u punti",
    "GPX esportato", "Atlante offline", "Nessuna tile nella cache",
    "Lettura atlante…", "POI", "Caricamento POI…", "%u POI trovati",
    "Altimetria: Open-Meteo · Copernicus DEM", "colore", "icona",
    "livello", "distanza", "sposta", "tile successiva", "apri mappa",
    "reimposta vista", "Menu principale", "Impostazioni", "Interfaccia",
    "Memoria e log", "Navigazione layer", "Vista livelli",
    "ingrandimento", "entra nel layer", "esci dal layer", "solo cache",
};
...
constexpr const char *kEnglish[] = {
    ...
    "HUD behavior", "Map scale", "Center crosshair", "Measurement units",
    "Interface animations", "Persistent logs on Vita storage", "Auto: 2.5 s",
    "Always visible", "With HUD", "Visible", "Hidden", "Imperial", "Metric", "Reduced",
    ...
    "start", "north", "rotate", "Hiking mode", "GPX import/export", "Inbox",
    "History", "import", "export", "refresh", "Imported %u points",
    "GPX exported", "Offline atlas", "No cached tiles", "Loading atlas…",
    "POI", "Loading POIs…", "%u POIs found",
    "Elevation: Open-Meteo · Copernicus DEM", "color", "icon",
    "layer", "spacing", "pan", "next tile", "open map", "reset view",
    "Main menu", "Settings", "Interface", "Storage and logs",
    "Layer navigation", "Layer overview", "view zoom", "enter layer",
    "leave layer", "cache only",
};
...
constexpr const char *kFrench[] = {
    ...
    "Langue de l’interface", "Comportement du HUD", "Échelle de la carte", "Viseur central",
    "Unités de mesure", "Animations de l’interface",
    "Journaux persistants sur le stockage Vita", "Auto : 2,5 s",
    "Toujours visible", "Avec le HUD", "Visible", "Masqué", "Impérial", "Métrique",
    ...
    "pivoter", "Mode randonnée", "Import/export GPX", "Boîte de réception",
    "Historique", "importer", "exporter", "actualiser", "%u points importés",
    "GPX exporté", "Atlas hors ligne", "Aucune tuile en cache",
    "Chargement de l’atlas…", "POI", "Chargement des POI…", "%u POI trouvés",
    "Altitude : Open-Meteo · Copernicus DEM", "couleur", "icône",
    "calque", "espacement", "déplacer", "tuile suivante",
    "ouvrir la carte", "réinitialiser", "Menu principal", "Paramètres",
    "Interface", "Stockage et journaux", "Navigation du calque",
    "Vue des calques", "zoom d’affichage", "entrer dans le calque",
    "quitter le calque", "cache uniquement",
};
...
constexpr const char *kSpanish[] = {
    ...
    "Comportamiento del HUD", "Escala del mapa", "Mira central", "Unidades de medida",
    "Animaciones de la interfaz", "Registro persistente en el almacenamiento Vita",
    "Auto: 2,5 s", "Siempre visible", "Con el HUD", "Visible", "Oculto", "Imperial", "Métrico",
    ...
    "inicio", "norte", "girar", "Modo senderismo", "Importar/exportar GPX",
    "Entrada", "Historial", "importar", "exportar", "actualizar",
    "%u puntos importados", "GPX exportado", "Atlas sin conexión",
    "No hay teselas en caché", "Cargando atlas…", "POI", "Cargando POI…",
    "%u POI encontrados", "Altitud: Open-Meteo · Copernicus DEM",
    "color", "icono", "capa", "separación", "mover",
    "siguiente tesela", "abrir mapa", "restablecer vista", "Menú principal",
    "Ajustes", "Interfaz", "Almacenamiento y registros",
    "Navegación de capa", "Vista de capas", "zoom de vista",
    "entrar en la capa", "salir de la capa", "solo caché",
};
...
constexpr const char *kGerman[] = {
    ...
    "UI-Sprache", "HUD-Verhalten", "Kartenmaßstab", "Fadenkreuz", "Maßeinheiten",
    "Oberflächenanimationen", "Dauerhafte Protokolle im Vita-Speicher",
    "Auto: 2,5 s", "Immer sichtbar", "Mit HUD", "Sichtbar", "Ausgeblendet", "Imperial",
    ...
    "Start", "Norden", "drehen", "Wandermodus", "GPX-Import/Export",
    "Eingang", "Verlauf", "importieren", "exportieren", "aktualisieren",
    "%u Punkte importiert", "GPX exportiert", "Offline-Atlas",
    "Keine Kacheln im Cache", "Atlas wird geladen…", "POI",
    "POIs werden geladen…", "%u POIs gefunden",
    "Höhe: Open-Meteo · Copernicus DEM", "Farbe", "Symbol",
    "Ebene", "Abstand", "verschieben", "nächste Kachel",
    "Karte öffnen", "Ansicht zurücksetzen", "Hauptmenü", "Einstellungen",
    "Oberfläche", "Speicher und Protokolle", "Ebenennavigation",
    "Ebenenübersicht", "Ansichtszoom", "Ebene öffnen", "Ebene verlassen",
    "nur Cache",
};
...
constexpr const char *kPortuguese[] = {
    ...
    "Comportamento do HUD", "Escala do mapa", "Mira central", "Unidades de medida",
    "Animações da interface", "Registos persistentes no armazenamento da Vita",
    "Auto: 2,5 s", "Sempre visível", "Com o HUD", "Visível", "Oculto", "Imperial", "Métrico",
    ...
    "início", "norte", "rodar", "Modo caminhada", "Importar/exportar GPX",
    "Entrada", "Histórico", "importar", "exportar", "atualizar",
    "%u pontos importados", "GPX exportado", "Atlas offline",
    "Sem mosaicos em cache", "A carregar atlas…", "POI", "A carregar POI…",
    "%u POI encontrados", "Altitude: Open-Meteo · Copernicus DEM",
    "cor", "ícone", "camada", "espaçamento", "mover",
    "próximo mosaico", "abrir mapa", "repor vista", "Menu principal",
    "Definições", "Interface", "Armazenamento e registos",
    "Navegação da camada", "Vista das camadas", "zoom da vista",
    "entrar na camada", "sair da camada", "apenas cache",
};
...
constexpr const char *kJapanese[] = {
    ...
    "地図の縮尺", "中央の照準", "単位", "画面アニメーション", "Vitaストレージへのログ保存",
    "自動: 2.5秒", "常に表示", "HUDと表示", "表示", "非表示", "ヤード・ポンド法",
    ...
    "回転", "ハイキングモード", "GPXインポート/エクスポート", "受信箱",
    "履歴", "インポート", "エクスポート", "更新", "%u地点をインポート",
    "GPXを書き出しました", "オフラインアトラス", "キャッシュタイルなし",
    "アトラス読み込み中…", "POI", "POI読み込み中…", "%u件のPOI",
    "標高: Open-Meteo · Copernicus DEM", "色", "アイコン",
    "レイヤー", "間隔", "移動", "次のタイル", "地図を開く",
    "表示をリセット", "メインメニュー", "設定", "インターフェース",
    "ストレージとログ", "レイヤー移動", "レイヤー一覧", "表示ズーム",
    "レイヤーに入る", "レイヤーを出る", "キャッシュのみ",
};
...
constexpr const char *kKorean[] = {
    ...
    "HUD 동작", "지도 축척", "중앙 조준선", "측정 단위", "인터페이스 애니메이션",
    "Vita 저장소에 로그 저장", "자동: 2.5초", "항상 표시", "HUD와 함께", "표시", "숨김",
    ...
    "회전", "하이킹 모드", "GPX 가져오기/내보내기", "받은 파일", "기록",
    "가져오기", "내보내기", "새로 고침", "%u개 지점 가져옴",
    "GPX 내보냄", "오프라인 아틀라스", "캐시된 타일 없음",
    "아틀라스 불러오는 중…", "POI", "POI 불러오는 중…", "%u개 POI",
    "고도: Open-Meteo · Copernicus DEM", "색상", "아이콘",
    "레이어", "간격", "이동", "다음 타일", "지도 열기", "보기 초기화",
    "메인 메뉴", "설정", "인터페이스", "저장소 및 로그", "레이어 탐색",
    "레이어 개요", "보기 확대", "레이어 들어가기", "레이어 나가기",
    "캐시 전용",
};
...
constexpr const char *kChineseSimplified[] = {
    ...
    "地图比例尺", "中心准星", "测量单位", "界面动画", "在Vita存储中保存日志",
    "自动: 2.5秒", "始终显示", "随HUD显示", "显示", "隐藏", "英制", "公制",
    "减少", "流畅", "启用", "禁用", "已保存",
    "错误: 0x%08X", "选择/更改", "更改", "返回", "退出",
    "收藏夹", "地点%u", "地图缓存", "读取中…", "%s · %u个瓦片",
    "再次按X清除", "正在清除…", "缓存已清除", "缓存错误: 0x%08X", "清除",
    "显示/隐藏", "闭合路径", "打开路径", "已闭合",
    "至少需要3个点才能闭合路径", "起点", "北", "旋转", "徒步模式",
    "GPX导入/导出", "收件箱", "历史", "导入", "导出", "刷新",
    "已导入%u个点", "GPX已导出", "离线图集", "缓存中没有瓦片",
    "正在加载图集…", "POI", "正在加载POI…", "找到%u个POI",
    "海拔: Open-Meteo · Copernicus DEM", "颜色", "图标",
    "图层", "间距", "平移", "下一张图块", "打开地图", "重置视图",
    "主菜单", "设置", "界面", "存储与日志", "图层导航", "图层总览",
    "视图缩放", "进入图层", "退出图层", "仅缓存",
};
...
constexpr const char *kChineseTraditional[] = {
    ...
    "地圖比例尺", "中心準星", "測量單位", "介面動畫", "在Vita儲存空間保存日誌",
    "自動: 2.5秒", "永遠顯示", "隨HUD顯示", "顯示", "隱藏", "英制", "公制",
    "減少", "流暢", "啟用", "停用", "已儲存",
    "錯誤: 0x%08X", "選擇/更改", "更改", "返回", "退出",
    "我的最愛", "地點%u", "地圖快取", "讀取中…", "%s · %u個圖磚",
    "再次按X清除", "正在清除…", "快取已清除", "快取錯誤: 0x%08X", "清除",
    "顯示/隱藏", "閉合路徑", "開啟路徑", "已閉合",
    "至少需要3個點才能閉合路徑", "起點", "北", "旋轉", "健行模式",
    "GPX匯入/匯出", "收件匣", "歷史", "匯入", "匯出", "重新整理",
    "已匯入%u個點", "GPX已匯出", "離線圖集", "快取中沒有圖磚",
    "正在載入圖集…", "POI", "正在載入POI…", "找到%u個POI",
    "海拔: Open-Meteo · Copernicus DEM", "顏色", "圖示",
    "圖層", "間距", "平移", "下一個圖塊", "開啟地圖", "重設視圖",
    "主選單", "設定", "介面", "儲存與日誌", "圖層導覽", "圖層總覽",
    "檢視縮放", "進入圖層", "離開圖層", "僅快取",
};
...
constexpr const char *kRussian[] = {
    ...
    "Поведение HUD", "Масштаб карты", "Центральный прицел", "Единицы измерения",
    "Анимации интерфейса", "Сохранять журнал на Vita", "Авто: 2,5 с",
    "Всегда видим", "Вместе с HUD", "Видим", "Скрыт", "Имперские", "Метрические",
    ...
    "начало", "север", "вращать", "Походный режим", "Импорт/экспорт GPX",
    "Входящие", "История", "импорт", "экспорт", "обновить",
    "Импортировано точек: %u", "GPX экспортирован", "Офлайн-атлас",
    "В кэше нет тайлов", "Загрузка атласа…", "POI", "Загрузка POI…",
    "Найдено POI: %u", "Высота: Open-Meteo · Copernicus DEM",
    "цвет", "значок", "слой", "интервал", "сдвиг",
    "следующий тайл", "открыть карту", "сбросить вид", "Главное меню",
    "Настройки", "Интерфейс", "Хранилище и журналы", "Навигация по слою",
    "Обзор слоёв", "масштаб вида", "войти в слой", "выйти из слоя",
    "только кэш",
};
