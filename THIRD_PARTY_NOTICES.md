# Third-party notices

VitaMaps links the following independently maintained components. Refer to
the installed VitaSDK packages and the `external/vita-https` submodule for the
complete corresponding license texts and source provenance.

- [vita-https](https://github.com/spyro-98/vita-https) — GPL-3.0-only.
- libcurl — curl license.
- Mbed TLS — Apache-2.0 OR GPL-2.0-or-later.
- Mozilla CA/certifi bundle — MPL-2.0.
- vita2d — MIT.
- libpng — libpng license.
- zlib — zlib license.
- FreeType — FreeType Project License or GPL-2.0-only.
- Inter 4.1 — SIL Open Font License 1.1; the license is packaged at
  `app0:licenses/Inter-OFL.txt`.
- libjpeg-turbo — BSD-3-Clause and IJG terms.
- bzip2 — bzip2 license.
- pthread-embedded — LGPL-2.1 with Vita-specific terms in VitaSDK.
- Natural Earth `ne_10m_populated_places_simple` 5.1.2 — public-domain
  populated-place data used to generate the compact native local-search index;
  source and terms:
  https://www.naturalearthdata.com/downloads/10m-cultural-vectors/10m-populated-places/
  and https://www.naturalearthdata.com/about/terms-of-use/.

Map data is © OpenStreetMap contributors and is subject to the
[OpenStreetMap copyright terms](https://www.openstreetmap.org/copyright) and
[tile usage policy](https://operations.osmfoundation.org/policies/tiles/).
Community raster styles additionally retain their own attribution and terms:
CyclOSM and the OSM France/Humanitarian renderings are credited in the native
map overlay and documented in the project README. OpenTopoMap is credited in
the same mandatory overlay; its rendered map is distributed under CC BY-SA 3.0
and combines OpenStreetMap data with SRTM/Copernicus elevation sources. See
https://dev.opentopomap.org/about.

Hiking elevation profiles use the Open-Meteo Elevation API and Copernicus
GLO-90 elevation data. The source is credited in the native list screen and is
documented at https://open-meteo.com/en/docs/elevation-api. Nearby points of
interest are returned by the public OpenStreetMap Overpass API; OpenStreetMap
attribution remains visible in the map renderer.

The application icon is derived from NASA's Apollo 17 “Blue Marble” photograph
AS17-148-22727. Image credit: NASA Johnson Space Center. The icon contains no
NASA identifier or logo and does not imply NASA endorsement. See the official
[image source](https://science.nasa.gov/resource/the-blue-marble/) and
[NASA image and media usage guidelines](https://www.nasa.gov/nasa-brand-center/images-and-media/).
